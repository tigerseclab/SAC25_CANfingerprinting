...
def CNN(dataset, type, size):
    ...
    # Reshape input based on the number of features
    features = X.reshape((X.shape[0], X.shape[1], 1))
    target = y_one_hot

    # Compute class weights (to mimic class_weight="balanced")
    ...
    def create_cnn_model():
        model = Sequential([
            Conv1D(32, 3, activation='relu', input_shape=(features.shape[1], 1)),
    ...
    # Set the number of models for ensemble approach
    ...
    for _ in range(n_models):
        X_resampled, y_resampled = resample(features, target, replace=True, n_samples=len(features))
